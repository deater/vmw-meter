...
int sine_demo(int i2c_fd, int device_num) {
	...
	int out_buffer_red[XSIZE][YSIZE];
	int out_buffer_green[XSIZE][YSIZE];

	unsigned char display_buffer_red[DISPLAY_LINES];
	unsigned char display_buffer_green[DISPLAY_LINES];

	for(x=0;x<XSIZE;x++) for (y=0;y<YSIZE;y++) {
		out_buffer_red[x][y]=0;
		out_buffer_green[x][y]=0;
	}
...
//		printf("%d %d %lf %lf\n",x,y,temp_dx,dy);
	...
	for(y=0;y<YSIZE;y++) {
		...
		display_buffer_red[y]=0;
		display_buffer_green[y]=0;
		for(x=0;x<XSIZE;x++) {
			if (out_buffer_red[x][y]) {
				plotxy(display_buffer_red,x,y);
			}

			if (out_buffer_green[x][y]) {
				plotxy(display_buffer_green,x,y);
			}
		}
	}
